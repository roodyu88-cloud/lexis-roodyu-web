...
export async function POST(req: Request) {
    ...
    try {
        ...
        systemPrompt += `\n\n[КРИТИЧЕСКОЕ ПРАВИЛО ПОИСКА]: Часто в законах нумерация идет без слова "Статья" (например, просто "2.5.2" или "ч.1"). Если пользователь спрашивает про статью 2.5.2, ты ОБЯЗАН найти в тексте строку "2.5.2" и выдать ее содержимое. НИКОГДА не отвечай, что статья отсутствует, пока не проверишь текст на наличие просто цифр (например, 2.5.2). Будь максимально внимателен к каждой цифре.`;
