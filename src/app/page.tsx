...
export default function Home() {
  ...
  return (
    <main className="min-h-screen w-full bg-[var(--color-void-black)]">
      ...
      {/* Hero pin: sticky wrapper keeps the hero fixed in the viewport for an
          extra scroll run while the advantages section rises over it — the
          "pinned reveal" transition requested to replace the abrupt cut. */}
      <div className="rc-hero-pin w-full">
        <section className="rc-hero w-full flex flex-col items-center text-center px-8">
          <div className="rc-hero-wash" aria-hidden="true" />
          <div className="rc-hero-beams" aria-hidden="true" />

          <div className="rc-hero-content max-w-3xl lg:max-w-5xl w-full flex flex-col items-center space-y-6">
            ...
            <div className="flex flex-col lg:flex-row gap-3 mt-4 flex-wrap justify-center">
              ...
              <Link href="/presets" className="rc-btn-ghost whitespace-nowrap">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"></path></svg>
                База Пресетов
              </Link>
              <a href={discordUrl} target="_blank" rel="noreferrer" className="rc-btn-ghost whitespace-nowrap">
              ...
              <Link href="/exam" className="rc-btn-ghost whitespace-nowrap">
                <GraduationCap className="w-4 h-4" />
                Тренажер
                <span className="rc-badge" style={{ background: "var(--color-coral-pulse)", color: "var(--color-on-coral)" }}>PREMIUM</span>
              </Link>
              {session?.user && (
                <Link href="/upload" className="rc-btn-ghost whitespace-nowrap">
      ...
      {/* Advantages: unified Void-referenced numbered accordion — replaces the
          previous two mismatched card grids with one consistent list, rising
          over the pinned hero as the reveal's leading edge. */}
      <section className="rc-hero-reveal max-w-4xl w-full mx-auto px-8 pt-16 pb-20">
        <Reveal className="text-center mb-12">
          <span className="rc-eyebrow" style={{ color: "var(--color-coral-text)" }}>Преимущества</span>
          <h2 className="text-heading font-bold text-[var(--color-pure-white)] mt-3 mb-3">Всё необходимое в одном оверлее.</h2>
          ...
          <div className="relative z-10 p-3 sm:p-6">
            <div className="rounded-2xl overflow-hidden" style={{ background: "color-mix(in srgb, var(--color-ink) 92%, transparent)", border: "1px solid var(--color-hairline)", backdropFilter: "blur(20px)" }}>
              ...
              <div className="p-6 text-left space-y-4">
                <div className="font-semibold text-[var(--color-pure-white)]">Могут ли меня уволить за халатность, если я не читал инструкцию?</div>
                <div className="text-sm text-[var(--color-ash)] leading-relaxed space-y-2">
                  <p>Да, незнание должностной инструкции не освобождает от ответственности по УК сервера Majestic (ст. 15.6 «Халатность»), если бездействие привело к ущербу для фракции или третьих лиц:</p>
                  <p>1. Руководитель фракции фиксирует факт бездействия рапортом.<br />2. Определяется размер причиненного ущерба.<br />3. Назначается взыскание — от выговора до увольнения с занесением в личное дело.</p>
                  <p>Если ущерба не было, наказание ограничивается предупреждением.</p>
                </div>
                <div className="flex items-center gap-2 pt-2" style={{ borderTop: "1px solid var(--color-hairline)" }}>
                  <span className="flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full" style={{ background: "var(--overlay-soft)", color: "var(--color-ash)", border: "1px solid var(--color-hairline)" }}>
                    <ShieldCheck className="w-3 h-3" /> УК Majestic, ст. 15.6
                  </span>
                </div>
              </div>
            </div>
