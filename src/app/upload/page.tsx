  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch("/api/upload", {
        ...
        body: JSON.stringify({ name, description, data: text, serverId: selectedServerId || null })
      });
              ...
              <>
                ...
                <div className="absolute left-0 right-0 mt-2 bg-[#0d0e12]/95 border border-white/10 rounded-xl overflow-hidden z-20 shadow-2xl animate-scale-up backdrop-blur-xl max-h-60 overflow-y-auto no-scrollbar">
                  ...
                  {Object.entries(servers.reduce((acc, s) => {
                      const proj = s.projectName || "Другое";
                      if (!acc[proj]) acc[proj] = [];
                      acc[proj].push(s);
                      return acc;
                  }, {} as Record<string, any[]>)).map(([projName, projServers]) => (
                    <div key={projName}>
                      <div className="px-4 py-1.5 text-xs font-bold text-gray-500 uppercase tracking-wider bg-black/40">{projName}</div>
                      {(projServers as any[]).map((s: any) => (
                        <button
                          key={s.id}
                          type="button"
